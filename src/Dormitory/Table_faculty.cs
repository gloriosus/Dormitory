...
namespace Dormitory
{
    public partial class Table_faculty : Form
    {
        public Table_faculty()
        {
            InitializeComponent();
        }
        ...
        private void Table_country_Load(object sender, EventArgs e)
        {
            ...
            UpdateTable("SELECT id, title FROM faculty WHERE is_active = true");
        }
        ...
        private void ButtonAdd_Click(object sender, EventArgs e)
        {
            ...
            if (nominal != "")
            {
                db.Set("INSERT INTO faculty (title) VALUES ('" + nominal + "')");
                UpdateTable("SELECT id, title FROM faculty WHERE is_active = true");
            }
        }
        ...
        private void ButtonDelete_Click(object sender, EventArgs e)
        {
            if (Table.CurrentRow.Index != -1)
            {
                ...
                db.Set("UPDATE faculty SET is_active = false WHERE id=" + id);
                UpdateTable("SELECT id, title FROM faculty WHERE is_active = true");
            }
        }
        ...
        private void ButtonEdit_Click(object sender, EventArgs e)
        {
            ...
            if (nominal != "")
            {
                ...
                db.Set("UPDATE faculty SET title='" + nominal + "' WHERE id=" + id);
                UpdateTable("SELECT id, title FROM faculty WHERE is_active = true");
            }
        }
        ...
        private void ButtonFind_Click(object sender, EventArgs e)
        {
            if (FieldSearch.Text != "")
            {
                searchQuery = "SELECT id, title FROM faculty WHERE is_active = true AND " + categories[FieldSearchCategory.SelectedIndex] + "='" + FieldSearch.Text + "'";
        ...
        private void ButtonSort_Click(object sender, EventArgs e)
        {
            if (FieldSortCategory.SelectedIndex != -1 && FieldSortOrder.SelectedIndex != -1)
            {
                ...
                if (!isSearched)
                {
                    query = "SELECT id, title FROM faculty WHERE is_active = true ORDER BY " + categories[FieldSortCategory.SelectedIndex] + " " + orders[FieldSortOrder.SelectedIndex];
                }
                else
                {
