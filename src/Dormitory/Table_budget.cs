...
namespace Dormitory
{
    public partial class Table_budget : Form
    {
        public Table_budget()
        {
            InitializeComponent();
        ...
        private void Table_country_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                UpdateTable("SELECT id, title FROM budget WHERE is_active = true");
            }
            catch
            {
        ...
        private void ButtonAdd_Click(object sender, EventArgs e)
        {
            ...
            if (nominal != "")
            {
                try
                {
                    db.Set("INSERT INTO budget (title) VALUES ('" + nominal + "')");
                    UpdateTable("SELECT id, title FROM budget WHERE is_active = true");
                }
                catch
                {
        ...
        private void ButtonDelete_Click(object sender, EventArgs e)
        {
            if (Table.CurrentRow.Index != -1)
            {
                ...
                try
                {
                    db.Set("UPDATE budget SET is_active = false WHERE id=" + id);
                    UpdateTable("SELECT id, title FROM budget WHERE is_active = true");
                }
                catch
                {
        ...
        private void ButtonEdit_Click(object sender, EventArgs e)
        {
            ...
            if (nominal != "")
            {
                ...
                try
                {
                    db.Set("UPDATE budget SET title='" + nominal + "' WHERE id=" + id);
                    UpdateTable("SELECT id, title FROM budget WHERE is_active = true");
                }
                catch
                {
        ...
        private void ButtonFind_Click(object sender, EventArgs e)
        {
            if (FieldSearch.Text != "")
            {
                searchQuery = "SELECT id, title FROM budget WHERE is_active = true AND " + categories[FieldSearchCategory.SelectedIndex] + "='" + FieldSearch.Text + "'";
        ...
        {
            FieldSearch.Text = "";
            UpdateTable("SELECT id, title FROM budget FROM is_active = true");
            isSearched = false;
        }
        ...
        private void ButtonSort_Click(object sender, EventArgs e)
        {
            if (FieldSortCategory.SelectedIndex != -1 && FieldSortOrder.SelectedIndex != -1)
            {
                ...
                if (!isSearched)
                {
                    query = "SELECT id, title FROM budget WHERE is_active = true ORDER BY " + categories[FieldSortCategory.SelectedIndex] + " " + orders[FieldSortOrder.SelectedIndex];
                }
                else
                {
